...
| apisix_llm_active_connections         | gauge     | Gauge of in-flight LLM upstream attempts. APISIX increments it when an attempt starts and decrements it when that attempt ends, including before an `ai-proxy-multi` fallback retry. See the [label reference](#labels-for-apisix_llm_active_connections) for details. |
...
APISIX increments this gauge when an LLM upstream attempt starts and decrements it when that attempt ends. On `ai-proxy-multi` fallback retry, the failed attempt is decremented before the next instance is tried, so each series tracks in-flight attempts to that instance. The final attempt is decremented in the request log phase.
